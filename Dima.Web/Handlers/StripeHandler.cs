    ...
    public class StripeHandler(IHttpClientFactory httpClientFactory) : IStripeHandler
    {
        ...
        public async Task<Response<string?>> CreateSessionAsync(CreateSessionRequest request)
        {
            ...
            return await result.Content.ReadFromJsonAsync<Response<String?>>()
                ?? new Response<string?>(null, 400, "Falha ao criar sessao no Stripe");
        }

        public async Task<Response<List<StripeTransactionResponse>?>> GetTransactionByOrderNumberAsync(GetTransactionByOrderNumberRequest request)
        {
            ...
            ?? new Response<List<StripeTransactionResponse>?>(null, 400, "Falha ao consultar transacoes do pedido");
        }
    }
}
